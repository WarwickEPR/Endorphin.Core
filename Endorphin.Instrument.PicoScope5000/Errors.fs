...
/// Represents an exception due to a PicoScope driver call error status.
/// Format: error message, status, action description.
exception PicoException of (string * PicoStatus * string)

[<AutoOpen>]
/// Adds extension members to PicoStatus enum.
module ErrorExtensions =
    type PicoStatus with

        /// Provides a message string describing the PicoStatus.
        member status.Message = 
            ...
            | status -> sprintf "Failed with unexpected PicoStatus value: %d." (int status)

        /// Returns an optional value which is either None in the case of PicoStatus.Ok or takes the value of the message for
        /// the status otherwise.
        member status.ErrorMessage =
            match status with
            | PicoStatus.Ok -> None
            | _ -> Some status.Message

        /// Returns a new PicoException with the appropriate error message and the provided action description.
        member status.Exception action =
            PicoException (status.Message, status, action)