...
type PicoScope5000(sessionParams) =
    ...
    let checkStatus action (status : PicoStatus) =
        ...
        | None -> ()
        | Some error ->
            failwithf "PicoScope %s command %s failed: %s." (sessionParams.SerialNumber) (action.ToString()) error
    ...
    let agent = Agent.Start(fun mailbox ->
        ...
        and stream dataBuffers = async {
            ...
            try
                ...
                if message.IsNone then // if no message was received, stop the acquisition
                    failwithf "PicoScope %s streaming acquisition failed due to timeout." sessionParams.SerialNumber
             
                (sprintf "PicoScope %s received messsage %A." sessionParams.SerialNumber message.Value) |> log.Info
